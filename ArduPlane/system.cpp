    ...
    case AUTO:
        ...
        if ((g.fence_autoenable == AUTOMODE_GFENABLE ||
            ...
            if (! geofence_set_enabled(true, AUTO_TOGGLED)) {
                gcs().send_text(MAV_SEVERITY_WARNING, "Enable fence on auto mode failed (cannot autoenable)");
            } else {
                gcs().send_text(MAV_SEVERITY_INFO, "Fence enabled by entering auto mode");
            }
        }
#endif
        throttle_allows_nudging = true;
